        ...
        messageListener = ListenerService.shared.messagesObserve(chat: chat, completion: { (result) in
            ...
            case .success(var message):
                if let url = message.downloadURL {
                    StorageService.shared.dowloadImage(url: url) { [weak self] (result) in
    ...
    private func sendImage(image: UIImage) {
        StorageService.shared.uploadImageMessage(photo: image, to: chat) { (result) in
            ...
            case .success(let url):
                ...
                FireStoreServices.shared.sendMessage(chat: self.chat, message: message) { (result) in
                    switch result {
                    case .success():
...
extension ChatsViewController {
    ...
    func configureCameraIcon() {
        ...
        cameraItem.tintColor = #colorLiteral(red: 0.7749517483, green: 0.5967955249, blue: 0.867061688, alpha: 1)
...
extension ChatsViewController: InputBarAccessoryViewDelegate {
    func inputBar(_ inputBar: InputBarAccessoryView, didPressSendButtonWith text: String) {
        ...
        FireStoreServices.shared.sendMessage(chat: chat, message: message) { (result) in
            switch result {
            case .success:
                self.messagesCollectionView.scrollToLastItem()
            case .failure(let error):
